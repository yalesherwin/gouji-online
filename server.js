...
function broadcastRoom(roomCode) {
  ...
  if (!room) return;
  const seats = room.players.map((p, i) => ({ seat:i+1, id:p.id, name:p.name, ready:p.ready, cards:p.hand.length, qualified: p.qualified }));
...
io.on('connection', (socket) => {
  socket.on('room:create', ({ name, mode='不开点' }) => {
    ...
    const room = { roomCode:c, mode, players:[], started:false, logs:['房间创建成功'], tributeRule:true };
    rooms.set(c, room);
  ...
  socket.on('room:join', ({ roomCode, name }) => {
    ...
    const player = { id: socket.id, name: name || `玩家${room.players.length+1}`, ready:false, hand:[], qualified:false };
